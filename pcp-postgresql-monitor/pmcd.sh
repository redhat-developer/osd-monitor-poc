...
echo "postgresql      110     pipe    binary       python /var/lib/pcp/pmdas/postgresql/pmdapostgresql.python -l -" >> $PCP_PMCDCONF_PATH;
...
# pcp >= 4.2ish
conf=pmdapostgresql.conf
echo '[authentication]' > $conf
echo "host=$DB_HOSTNAME" >> $conf
echo "port=$DB_PORT" >> $conf
echo "dbname=$DB_DB" >> $conf
echo "user=$DB_USER" >> $conf
echo "password=$DB_PASSWORD" >> $conf
