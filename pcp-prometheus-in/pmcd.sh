...
echo "prometheus 144 pipe binary env PCP_DEBUG=all python $PCP_PMDAS_DIR/prometheus/pmdaprometheus.python -D -l -" >> $PCP_PMCDCONF_PATH;
...
: "${PMCD_TIMEOUT:=300}"
exec /usr/libexec/pcp/bin/pmcd -l - -f -t $PMCD_TIMEOUT -p $PMCD_PORT -A -Dall -H $PCP_HOSTNAME
